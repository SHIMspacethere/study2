...
RANKING_FILE = "ranking.txt"   # 순위 데이터 저장에 사용할 파일 이름 정의
# 파일에서 읽어서 순위 데이터 초기화(파일이 있는 경우)
ranking = {}   
if os.path.exists(RANKING_FILE):
    with open(RANKING_FILE, "r") as f:
        for line in f:
            name, score = line.strip().split(":")
            ranking[name] = int(score)
...
def runRanking( ) :
  global shotCount
  name = "$$$"
  input( "input ranking name : ", name )
  # 새로운 점수가 순위에 추가되면 데이터 구조를 업데이트하고 파일에 다시 씁니다.
  ranking[name] = shotCount
