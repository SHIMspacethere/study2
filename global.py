...
# 첫 화면( 시놉시스 ) 구현
def runStory( ) :
  global background, background2, fighter, missile, explosion, missileSound, gameOverSound, clock, shotCount, background_speed, x, y
...
# 시작화면 출력
def runMenu( ) :
    global BLACK, padWidth, padHeight, background_height, rockImage, explosionSound, screen, start_time, gamePad, x, y
    global background, background2, fighter, missile, explosion, missileSound, gameOverSound, clock, shotCount, background_speed
    ...
    textpos1.center = ( padWidth / 2, padHeight / 2 )
    textpos2 = text2.get_rect( )
    textpos2.center = ( padWidth / 2, padHeight / 2 + 30 )
    textpos3 = text3.get_rect( )
    textpos3.center = ( padWidth / 2, padHeight / 2 + 60 )
    ...
    while True :
      for event in pygame.event.get( ) :
        ...
        if event.type == pygame.KEYDOWN :
          ...
          if event.key == pygame.K_k : # 숫자 k 누르면 키보드 모드
            runGame_keyboard( )
          if event.key == pygame.K_m : # 숫자 m 누르면 마우스 모드
            runGame_mouse( )
          else:
            runGame_keyboard( )
...
def rockGen ( ) :
    ...
    rockAngle = 0
    rockAngle += 0.5  # 추가: 운석의 회전 속도 DJ
    if rockAngle >= 360:
        rockAngle = 0
    #rock = pygame.image.load( random.choice( rockImage ) )
    rotated_rock = pygame.transform.rotate(rock, rockAngle)
    rockA = rotated_rock.get_rect(left=random.randint(0, padWidth - rock.get_width()), top=-30)
    dy = random.randint(1, 2)
    #print(rockA)
    rocks.append((rockA, dy))
