...
# 첫 화면( 시놉시스 ) 구현
def runStory( ) :
  global gamePad, background, screen, shotCount
...
def rankingMain( gameScore ) :     
    # 게임점수가 랭킹에 포함되는지 체크
    def checkRanking( gameScore ) :
        import pymysql     
        rankingCnt = 5  # 보여줄 랭킹 갯수
        minScore = 0    # 랭킹순위중 가장 낮은 점수
        
        # MySQL Connection 연결
        conn = pymysql.connect( host='192.168.0.31', user='root', password='1234', db='pythonDB', charset='utf8' )    
        ...
        curs.execute( sql, ( 0, rankingCnt ) )    # Top 5만 출력  
    ...
    def showRangking() :
        ...
        # SQL문 실행
        #sql = "select * from GameRainkingTbl where category=%s and region=%s ;"
        sql = " select gr_name, gr_score, gr_date from gamerankingtbl order by gr_score desc Limit %s, %s ;"
        curs.execute(sql, (0, 5))   # Top 5만 출력  
    ...
    def insertRanking( gameScore ) :
        ...
        sql = "insert into gamerankingtbl(gr_name,gr_score) values('" + data1 + "'," + str(data2) + ");"
        ...
        showRangking()    # 등록후 리스트 새로운 점수 추가
        window.mainloop() # 추가한 리스트 새로고침 
    ...
    win_x = 370   # 창 넓이
    win_y = 194   # 창 높이
    tot_x = window.winfo_screenwidth()
    tot_y = window.winfo_screenheight()
    x_pos = int( ( tot_x / 2 ) - ( win_x / 2 ) )
    y_pos = int( ( tot_y / 2 ) - ( win_y / 2 ) )
    
    window.geometry(f"{win_x}x{win_y}+{x_pos}+{y_pos}")    
    
    window.resizable(width=False, height=False) # 창 크기 고정
    window.title("슈팅게임 순위 V0.2c")
    
    # 게임 점수가 Top 5에 속할 경우 등록 버튼 나오게 처리        
    padding_top = Label( window, text = "", width = 1, height = 1 )
    padding_top.grid( row = 0, column = 0, columnspan = 6 ) 
    
    if ( checkRanking( gameScore ) ) :       
        padding_left = Label( window, text = "", width = 1 )
        padding_left.grid( row = 1, column = 0 ) 
        
        label =Label( window, text = "이름입력", fg=BLACK, bg = WHITE )
        label.grid( row = 1, column = 1 ) 
        
        edt1 = Entry( window, width = 17, bg = WHITE, justify = LEFT )  
        edt1.grid( row = 1, column = 2 )
        
        btnInsert = Button(window, text="저 장", bg = WHITE, justify = LEFT
                           ,command = lambda score = gameScore : insertRanking ( score ) )                      
        btnInsert.grid( row = 1, column = 3 )
        
        msg = "게임점수는 " +str( gameScore )+"점"
        label =Label( window, text = msg , fg=BLACK, bg = WHITE )
    ...
    label =Label( window, text = "순위", width = 7, fg=WHITE, bg = BLACK, justify = LEFT )
    label.grid( row = 3, column = 1 ) 
    
    label =Label( window, text = "사용자 이름", width = 16, fg=WHITE, bg = BLACK, justify = LEFT )
    label.grid( row = 3, column = 2 ) 
    
    label =Label( window, text = "점 수", width = 5, fg=WHITE, bg = BLACK, justify = LEFT )
    label.grid( row = 3, column = 3 ) 
    
    label =Label( window, text = "날 짜", width = 16, fg=WHITE, bg = BLACK, justify = LEFT )
    label.grid( row = 3, column = 4 )
    
    padding_left = Label( window, text = "", width = 1 )
    padding_left.grid( row = 4, column = 0 ) 
    
    listData1 = Listbox( window,bg = 'white', width = 7, height = 5, justify = LEFT )
    listData1.grid( row = 4, column = 1 )
     
    listData2 = Listbox( window,bg = 'white', width = 17, height = 5, justify = LEFT )
    listData2.grid( row = 4, column = 2 )
    
    listData3 = Listbox( window,bg = 'white', width = 5, height = 5, justify = LEFT )
    listData3.grid( row = 4, column = 3 )
    
    listData4 = Listbox( window,bg = 'white', width = 17, height = 5, justify = LEFT )
    listData4.grid( row = 4, column = 4 )
    
    padding_right = Label( window, text = "", width = 1 )
